...
local function getLevel(commandName, default)
    return Config.CommandLevels[commandName] or default
end
...
-- Kick/Ban
CommandManager.RegisterCommand("kick", getLevel("kick", 40), function(executor, args)
...
CommandManager.RegisterCommand("ban", getLevel("ban", 60), function(executor, args)
...
CommandManager.RegisterCommand("gban", getLevel("gban", 80), function(executor, args)
    local BanManager = require(script.Parent.BanManager)
    local targetName = args[1]
...
CommandManager.RegisterCommand("history", getLevel("history", 40), function(executor, args)
...
-- Moderation
CommandManager.RegisterCommand("mute", getLevel("mute", 40), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
CommandManager.RegisterCommand("unmute", getLevel("unmute", 40), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
-- Movement/Stats
CommandManager.RegisterCommand("fly", getLevel("fly", 60), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
CommandManager.RegisterCommand("unfly", getLevel("fly", 60), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
CommandManager.RegisterCommand("noclip", getLevel("noclip", 60), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
CommandManager.RegisterCommand("clip", getLevel("noclip", 60), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
-- Communication
CommandManager.RegisterCommand("announce", getLevel("announce", 60), function(executor, args)
...
CommandManager.RegisterCommand("message", getLevel("message", 60), function(executor, args)
    local msg = table.concat(args, " ")
    ReplicatedStorage:WaitForChild("NexusAdmin_Message"):FireAllClients("Message", msg, executor.Name)
end, "Global message")

CommandManager.RegisterCommand("pm", getLevel("pm", 20), function(executor, args)
    local targets = getPlayers(args[1], executor)
...
-- Fun/Misc
CommandManager.RegisterCommand("explode", getLevel("explode", 60), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
-- Rank Management
CommandManager.RegisterCommand("rank", getLevel("rank", 80), function(executor, args)
...
CommandManager.RegisterCommand("unrank", getLevel("unrank", 80), function(executor, args)
    local RankManager = require(script.Parent.RankManager)
    local targetName = args[1]
...
-- Kill Command
CommandManager.RegisterCommand("kill", getLevel("kill", 20), function(executor, args)
    local targets = getPlayers(args[1], executor)
    for _, target in pairs(targets) do
...
-- Warning System
CommandManager.RegisterCommand("warn", getLevel("warn", 40), function(executor, args)
...
CommandManager.RegisterCommand("warnings", getLevel("warnings", 40), function(executor, args)
    local WarningManager = require(script.Parent.WarningManager)
    local targetName = args[1]
...
CommandManager.RegisterCommand("viewinfo", getLevel("viewinfo", 40), function(executor, args)
    local targetName = args[1]
...
CommandManager.RegisterCommand("bring", getLevel("bring", 60), function(executor, args)
    local targetName = args[1]
...
CommandManager.RegisterCommand("goto", getLevel("goto", 60), function(executor, args)
    local targetName = args[1]
...
-- World Settings
CommandManager.RegisterCommand("gravity", getLevel("gravity", 80), function(executor, args)
...
CommandManager.RegisterCommand("time", getLevel("time", 80), function(executor, args)
    local WorldManager = require(script.Parent.WorldManager)
    local val = args[1]
    if val then
...
CommandManager.RegisterCommand("globalwalkspeed", getLevel("globalwalkspeed", 80), function(executor, args)
    local WorldManager = require(script.Parent.WorldManager)
    local val = tonumber(args[1])
    if val then
...
CommandManager.RegisterCommand("globaljumppower", getLevel("globaljumppower", 80), function(executor, args)
    local WorldManager = require(script.Parent.WorldManager)
    local val = tonumber(args[1])
    if val then
...
CommandManager.RegisterCommand("bans", getLevel("bans", 40), function(executor, args)
    local BanManager = require(script.Parent.BanManager)
...
CommandManager.RegisterCommand("tag", getLevel("tag", 80), function(executor, args)
...
CommandManager.RegisterCommand("untag", getLevel("untag", 80), function(executor, args)
    local TagManager = require(script.Parent.TagManager)
    local RankManager = require(script.Parent.RankManager)
